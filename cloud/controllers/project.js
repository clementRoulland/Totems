module.exports = function ()
{
    ...
    function take(req, res)
    {
        ...
        function success(theProject)
        {
            ...
            //Old handler for user and stack column
            if (theProject.get('user') && theProject.get('user') != Parse.User.current() &&
                !userIsInStack(Parse.User.current(), theProject.get('stack'))) {
                // add user to stack
                theProject.get('stack').push(Parse.User.current());
                console.log("Project::take | " + Parse.User.current().get("username") + " add to stack for project " +
                            theProject.get('name'));
            } else if (!theProject.get('user')) {
                // take project
                theProject.set("user", Parse.User.current());
                console.log("Project::take | " + Parse.User.current().get("username") + " take project " +
                            theProject.get('name'));
            }
            theProject.save();
            }
    ...
    function release(req, res)
    {
        ...
        function success(theProject)
        {
            ...
            var position = userIsInStack(Parse.User.current(),theProject.get('requests'))-1;
            theProject.get('requests').splice(position, 1);

            if (theProject.get('user') && theProject.get('user').id == Parse.User.current().id) {
                console.log("Project::release | " + Parse.User.current().get("username") + " release project " +
                            theProject.get('name'));
                if (theProject.get('stack').length > 0) {
                    theProject.set('user', theProject.get('stack').shift());
                    console.log("Project::release | " + Parse.User.current().get("username") + " take project " +
                                theProject.get('name') + " from stack");
                } else {
                ...
                console.log("Project::release | " + Parse.User.current().get("username") + " release project " +
                            theProject.get('name') + " from stack " + indexInStack);
            }
            theProject.save();
    ...
    function getPosition(user, requests)
    {
        ...
        console.log("Project::getPosition | stack | ");
        console.log(requests);
        console.log("Project::getPosition | user | ");
        console.log(user);
        var isInStack = false;
        var indexInStack = 0;
        requests.forEach(function (it)
        {
            console.log("Project::getPosition | it.id | " + it.user.id);
            if (it.user.id == user.id) {
                console.log("Project::getPosition | it.id == user.id | " + it.user.id + " == " + user.id);
                isInStack = true;
            }
            if (!isInStack)indexInStack++;
        });
